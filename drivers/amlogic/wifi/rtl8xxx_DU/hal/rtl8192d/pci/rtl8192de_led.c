...
)
{
	...
	{	
		...
		case LED_PIN_LED0:
			LedCfg = rtw_read8(padapter, REG_LEDCFG2);  //LED0(actually it is LED2)
			if ((pHalData->EEPROMDID == 0x8176)||(pHalData->EEPROMDID == 0x8193))
				//BIT7 of REG_LEDCFG2 should be set to make sure we could emit the led2. By page. 21110119
				rtw_write8(padapter, REG_LEDCFG2, (LedCfg&0xf0)|BIT7|BIT5|BIT6); // SW control led0 on.
			else
				rtw_write8(padapter, REG_LEDCFG2, (LedCfg&0xf0)|BIT7|BIT5); // SW control led0 on.
			break;

		case LED_PIN_LED1:
			LedCfg = rtw_read8(padapter, REG_LEDCFG1);  //LED1
	...
	)
{
	...
	if((padapter->pwrctrlpriv.rf_pwrstate == rf_off) && (padapter->pwrctrlpriv.rfoff_reason > RF_CHANGE_BY_PS))
	{
		RT_TRACE(_module_rtl8712_led_c_, _drv_err_, (" Hw/Soft Radio Off, turn off Led\n"));
		return;
	}
...
//
void
rtl8192de_InitSwLeds(
	_adapter	*padapter
	)
{
	struct led_priv *pledpriv = &(padapter->ledpriv);

	pledpriv->LedControlHandler = LedControl8192DE;
