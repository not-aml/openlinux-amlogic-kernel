...
static int rtl8192de_init_rx_ring(_adapter * padapter)
{
...
static void rtl8192de_free_rx_ring(_adapter * padapter)
{
	struct recv_priv	*precvpriv = &padapter->recvpriv;
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct pci_dev	*pdev = pdvobjpriv->ppcidev;
...
static int rtl8192de_init_tx_ring(_adapter * padapter, unsigned int prio, unsigned int entries)
{
...
static void rtl8192de_free_tx_ring(_adapter * padapter, unsigned int prio)
{
	struct xmit_priv	*pxmitpriv = &padapter->xmitpriv;
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	struct pci_dev	*pdev = pdvobjpriv->ppcidev;
...
_func_enter_;
	...
	while (ring->qlen) {
		...
		struct xmit_buf	*pxmitbuf = rtl8192de_dequeue_xmitbuf(ring);
...
static void rtl8192de_init_desc_ring_var(_adapter * padapter)
{
	struct recv_priv	*precvpriv = &padapter->recvpriv;
	struct xmit_priv	*pxmitpriv = &padapter->xmitpriv;
	u8 i = 0;
...
_func_enter_;
	...
	rtl8192de_init_desc_ring_var(padapter);

	ret = rtl8192de_init_rx_ring(padapter);
	if (ret == _FAIL) {
		return ret;
	}
	...
	for (i = 0; i < PCI_MAX_TX_QUEUE_COUNT; i++) {
		ret = rtl8192de_init_tx_ring(padapter, i, pxmitpriv->txringcount[i]);
		if (ret == _FAIL)
			goto err_free_rings;
	}
...
err_free_rings:
	...
	rtl8192de_free_rx_ring(padapter);

	for (i = 0; i <PCI_MAX_TX_QUEUE_COUNT; i++)
		if (pxmitpriv->tx_ring[i].desc)
			rtl8192de_free_tx_ring(padapter, i);
...
static void rtl8192de_xmit_beacon(PADAPTER Adapter)
{
...
void rtl8192de_prepare_bcn_tasklet(void *priv)
{
	...
	rtl8192de_xmit_beacon(padapter);
}

static void rtl8192de_tx_isr(PADAPTER Adapter, int prio)
{
	...
	while(ring->qlen)
	{
		...
		pxmitbuf = rtl8192de_dequeue_xmitbuf(ring);
...
#if 0
s32	rtl8192de_interrupt(PADAPTER Adapter)
{
	RT_ISR_CONTENT	isr_content;
	int	ret = _SUCCESS;

	//read ISR: 4/8bytes
	InterruptRecognized8192DE(Adapter, &isr_content);
	...
	if (isr_content.IntArray[0] & IMR_BDOK) {
		...
		rtl8192de_tx_isr(Adapter, BCN_QUEUE_INX);
	}
	
	if (isr_content.IntArray[0] & IMR_BcnInt) {
		//DBG_8192C("prepare beacon for interrupt!\n");
		//tasklet_hi_schedule(&Adapter->recvpriv.irq_prepare_beacon_tasklet);
	}
...
static void rtl8192de_rx_mpdu(_adapter *padapter)
{
	struct recv_priv	*precvpriv = &padapter->recvpriv;
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	...
	while (count--)
	{
		...
		else
		{
			...
			rtl8192d_query_rx_desc_status(precvframe, pdesc);
			...
			//recvframe_pull(precvframe, pattrib->drvinfo_sz + pattrib->shift_sz);
...
)
{
	...
	rtw_write16(Adapter,REG_DBI_CTRL, (Offset & 0xFFC));
	rtw_write8(Adapter, REG_DBI_FLAG, BIT1|Direct);
	rtw_udelay_os(10);
	value = rtw_read32(Adapter, REG_DBI_RDATA);

	//RT_TRACE(COMP_IO, DBG_TRACE, ("MpReadPCIDwordDBI8192D offset 0x%x direct 0x%x value 0x%x\n", Offset, Direct, value));
...
Input:    u2Byte reg ----PCI Configuration Space offset 
		u4Byte value -----the value write to offset of PCI Configuration Space
		Direct: Write to PCI config space, or MAC reg(MAC0 or MAC1 for 92DE)
		
Output:  void

Date:   2009.11.26

Function: Write to PCI Configuration Space after page 1 for Windows XP 

***********************************************************************************/
void 
MpWritePCIDwordDBI8192D(
	IN	PADAPTER	Adapter,
	IN	u16	Offset,
	IN	u32	Value,
	IN	u8	Direct
)
{
	...
	//RT_TRACE(COMP_INIT, DBG_LOUD, ("MpWritePCIDwordDBI8192D offset 0x%x direct 0x%x value 0x%x\n", Offset, Direct, Value));	
	return ;
}
...
void rtl8192de_xmit_tasklet(void *priv)
{
	...
	rtl8192de_xmitframe_resume(padapter);
}
