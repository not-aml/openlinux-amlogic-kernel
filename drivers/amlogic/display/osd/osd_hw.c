...
{
//we set pal here ,no do it in interrupt .
//the reason is :maybe we set group of pal values once a time.
//it will be simple we do it here ,if do it in interurpt ,we will add 
//some sync mechanism.
    if (regno < 256) {
        u32 pal;
        pal = ((red   & 0xff) << 24) |
