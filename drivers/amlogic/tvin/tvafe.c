...
static void tvafe_canvas_init(unsigned int mem_start)
{
    int i = 0;
    unsigned int canvas_width  = 1920;
    unsigned int canvas_height = 1080;
    unsigned int decbuf_size   = 0x70000;
    unsigned int decbuf_start  = mem_start;

    for (i = 0; i<TVAFE_VF_POOL_SIZE; i++)
    {
...
void tvafe_start_dec(unsigned int mem_start, enum tvin_port_e port, enum tvin_sig_fmt_e fmt)
{
    /** init canvas and variables**/
    //init canvas
    tvafe_canvas_init(mem_start);
    tvafe_info.rd_canvas_index = 0xFF - (TVAFE_VF_POOL_SIZE + 2);
    tvafe_info.wr_canvas_index = 0;
    tvafe_info.param.port = port;
    tvafe_info.param.fmt = fmt;

    tvafe_source_muxing(&tvafe_info, port);
...
{
    vdin_dev_t *p = (vdin_dev_t*)para;

    printk("tvafe_notifier_callback, port = %d, format = %d ./n" ,
                p->para.port, p->para.fmt);
    if ((p->para.port < TVIN_PORT_VGA0) || (p->para.port > TVIN_PORT_SVIDEO7))
        return -1;

    switch (cmd) {
        case TVIN_EVENT_DEC_START:
            printk("tvafe decode start./n");
            tvafe_start_dec(p->mem_start, p->para.port, p->para.fmt);
